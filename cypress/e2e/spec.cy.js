...
describe('Testa as formas geometricas', () => {
  it('Verifica se tem os selectors como filho da div com o id App', () => {
    cy.visit('http://localhost:5173/')
    cy.get('#app').should('be.visible')
    cy.get('#app > #selectors').should('be.visible')
  })

  it('Verifica se os selectors tem 4 divs filhas', () => {
    cy.visit('http://localhost:5173/')
    cy.get('#app > #selectors').children().should('have.length', 4)
  });

  it('Verifica se as 4 divs filhas dos selectors tem a classe color', () => {
    cy.visit('http://localhost:5173/')
    cy.get('#app > #selectors > .color').should('have.length', 4)
  })

  it('Verifica se as 4 divs filhas dos selectors tem a classe color e se tem a borda preta', () => {
    cy.visit('http://localhost:5173/')
    cy.get('#app > #selectors > .color').should('have.css', 'border', '1px solid rgb(0, 0, 0)')
  })

  it('Verifica se a primeira e terceira div sao quadradas com 50px de altura e largura', () => {
    cy.visit('http://localhost:5173/')
    cy.get('#app > #selectors > .color').eq(0).should('have.css', 'width', '50px')
    cy.get('#app > #selectors > .color').eq(0).should('have.css', 'height', '50px')
    cy.get('#app > #selectors > .color').eq(2).should('have.css', 'width', '50px')
    cy.get('#app > #selectors > .color').eq(2).should('have.css', 'height', '50px')
  })

  it('Verifica se a segunda e quarta div sao circulares com 50px de altura e largura', () => {
    cy.visit('http://localhost:5173/')
    cy.get('#app > #selectors > .color').eq(1).should('have.css', 'width', '50px')
    cy.get('#app > #selectors > .color').eq(1).should('have.css', 'height', '50px')
    cy.get('#app > #selectors > .color').eq(1).should('have.css', 'border-radius', '50%')
    cy.get('#app > #selectors > .color').eq(3).should('have.css', 'width', '50px')
    cy.get('#app > #selectors > .color').eq(3).should('have.css', 'height', '50px')
    cy.get('#app > #selectors > .color').eq(3).should('have.css', 'border-radius', '50%')
  })
});

describe('Verifica se as formas Geometricas usam FlexBox', () => {
  it('Verifica se os selectors tem a propriedade display flex', () => {
    cy.visit('http://localhost:5173/')
    cy.get('#app > #selectors').should('have.css', 'display', 'flex')
  })

  it('Verifica se os selectors tem a propriedade flex-direction row', () => {
    cy.visit('http://localhost:5173/')
    cy.get('#app > #selectors').should('have.css', 'flex-direction', 'row')
  })

  it('Verifica se os selectors estao no centro da tela', () => {
    cy.visit('http://localhost:5173/')
    cy.get('#app > #selectors').should('have.css', 'justify-content', 'center')
  })
})
...
describe('Verifica se as formas geometricas possuem cores distintas', () => {
  it('Verifica se todas as formas geometricas possuem cores definidas', () => {
    cy.visit('http://localhost:5173/')
    cy.get('#app > #selectors > .color').each(($el) => {
  ...
  it('Verifica se a ultima forma geometrica possui a cor preta', () => {
    cy.visit('http://localhost:5173/')
    cy.get('#app > #selectors > .color').last().should('have.css', 'background-color', 'rgb(0, 0, 0)')
  })

  it('Verifica se a paleta nao possui cores repetidas', () => {
    cy.visit('http://localhost:5173/')
    cy.get('#app > #selectors > .color').then((colors) => {
...
describe('Verifica botao de gerar cores', () => {
  ...
  it('Verifica se o botao ao ser clicado renderiza cores diferentes na paleta', () => {
    ...
    cy.get('#app > #selectors > .color').then((colors) => {
      ...
      cy.get('#app > #selectors > .color').then((newColors) => {
...
describe('Verifica se a classe Selected alterna entre a paleta', () => {
  it('Verifica se a classe Selected existe', () => {
    cy.visit('http://localhost:5173/')
    cy.get('#app > #selectors > .color').first().should('have.class', 'selected')
  })

  it('Verifica se ao carregar a pagina, a primeira cor tem a classe selected e as outras nao tem', () => {
    cy.visit('http://localhost:5173/')
    cy.get('#app > #selectors > .color').first().should('have.class', 'selected')
    cy.get('#app > #selectors > .color').first().siblings().should('not.have.class', 'selected')
  })
  ...
  it('Verifica se ao clicar em uma cor, a classe selected e adicionada a cor clicada e removida da cor anterior', () => {
    ...
    cy.get('#app > #selectors > .color').first().should('have.class', 'selected')
    cy.get('#app > #selectors > .color').first().siblings().should('not.have.class', 'selected')
    cy.get('#app > #selectors > .color').eq(1).click()
    cy.get('#app > #selectors > .color').eq(1).should('have.class', 'selected')
    cy.get('#app > #selectors > .color').first().should('not.have.class', 'selected')
    cy.get('#app > #selectors > .color').eq(2).click()
    cy.get('#app > #selectors > .color').eq(2).should('have.class', 'selected')
    cy.get('#app > #selectors > .color').eq(1).should('not.have.class', 'selected')
  })
})
...
describe('Verifica se ao selecionar uma cor da paleta e clicar no board, ele muda a cor de fundo corretamente', () => {
  it('Verifica se ao selecionar uma cor e clicar no board, o board muda de cor', () => {
    ...
    cy.get('#app > #selectors > .color').last().click()
    // Clica no board
...
describe('Verifica o LocalStorage do Board', () => {
  ...
  it('Verifique se ao pintar o board com outras cores, a cor permanece', () => {
    ...
    // Guarde numa variavel as cores do board
    cy.get('#app > #selectors > .color').eq(1).then((color) => {
    ...
    // Guarde numa variavel as cores do board
    cy.get('#app > #selectors > .color').eq(2).then((color) => {
      let colorSelected = color[0].style.backgroundColor
      if(colorSelected === 'black' || colorSelected === '#000000' || colorSelected === '#000') {
        colorSelected = 'rgb(0, 0, 0)'
      }
